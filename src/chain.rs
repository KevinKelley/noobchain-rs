...
impl NoobChain {
	...
	pub fn dump_blocks(&self) {
		...
		for block in &self.blockchain {
			...
			for tx in &block.transactions {
				println!("  {}  {}", tx.transaction_id.as_hex_string(), tx.value);
			}
		}
	}
	...
	pub fn main() {
		...
		let mut genesis_transaction = Transaction::new(coinbase.public_key(), wallet_a.public_key(), 1000.0, vec!());
		genesis_transaction.generate_signature(&coinbase);  // manually sign..
		genesis_transaction.transaction_id = vec!(0u8);		// manually set the id..
		...
		noobchain.add_utxo(genesis_transaction.outputs[0].clone());
		noobchain.dump_utxo();
		noobchain.dump_blocks();
		println!("coinbase's balance is: {}", coinbase.get_balance(&mut noobchain));
		...
		println!("\nwallet_a is Attempting to send funds (40) to wallet_b...");
		...
		println!("\nwallet_a Attempting to send more funds (1000) than it has...");
		if let Some(transaction) = wallet_a.send_funds(&mut noobchain, wallet_b.public_key(), 1000.0) {
